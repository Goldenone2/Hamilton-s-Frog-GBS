...
```sh
iqtree2 -s HamFrogR08maxsnps1DP5.recode.min4.phy -m MFP
```
Then I run IQtree using my own conda environment, the GTR+G model with 1000 bootstraps.

I have no idea what subsitution model is best for my data, so we will use a model selection method outlined [here](http://www.iqtree.org/doc/Tutorial).
